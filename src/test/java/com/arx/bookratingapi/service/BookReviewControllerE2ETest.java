...
public class BookReviewControllerE2ETest {
  ...
  public static void configureProperties(DynamicPropertyRegistry registry) {
    registry.add("spring.datasource.url", ()-> postgresContainer.getJdbcUrl() + "book_rating_db?currentSchema=book_reviews");
  ...
  @Test
  @DisplayName("Method getDetailedBookReview returns successfully a detailed book review with average rating")
  @Order(3)
  void getDetailedBookReview_returns_detailedBookReview() throws Exception {
    ...
    // Run test Verify the results
    MvcResult apiResponseResult = mockMvc.perform(MockMvcRequestBuilders.get("/reviews/1"))
        .andExpect(status().isOk()).andReturn();

    assertThat(apiResponseResult.getResponse().getContentAsString()).contains("A great book to read");

  }

  @Test
  @DisplayName("Method getDetailedBookReview returns 404 not found if there is no any review for specific book id")
  @Order(4)
  void getDetailedBookReview_returns_notFound() throws Exception {

    // Setup
    String notExistingReviewBookId = "2";

    // Run test Verify the results
    mockMvc.perform(MockMvcRequestBuilders.get("/reviews/" + notExistingReviewBookId))
        .andExpect(status().isNotFound());
